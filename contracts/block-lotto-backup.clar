...
(define-public (draw-winner)
  (let ((st (var-get status))
    ...
    ;; Only creator can draw, or anyone after target block is reached
    (asserts! (or (is-eq tx-sender admin) (>= current-height target)) (err ERR-NOT-ADMIN))
    
    ;; Use a recent block for randomness (previous block to avoid manipulation)
    (let ((random-block (- current-height u1)))
      (let ((header-hash (unwrap! (get-block-info? id-header-hash random-block) (err ERR-INVALID-STATUS))))
        ...
        (let ((random-hash (hash160 header-hash)))
          (let ((random-seed (+ (len random-hash) random-block current-height)))
